				  ...
				  local ok = vRP.request(target,lang.mcharge.request({my_directory_name,charge}),600)
				    if ok then
		              ...
		              if target_bank>0 then
						...
						TriggerEvent("logs:serverLog", "ID "..user_id.." cobrou por telefone "..charge.." para o ID "..target_id, "dinheiro", false)
		...
		if target_id~=nil then
          if my_bank >= 0 then
			...
			if target ~= nil then
			  ...
			  TriggerEvent("logs:serverLog", "ID "..user_id.." enviou por telefone "..transfer.." para o ID "..target_id, "dinheiro", false)
			  local identity = vRP.getUserIdentity(user_id)
		        local my_directory_name = vRP.getPhoneDirectoryName(target_id, identity.phone)
			    if my_directory_name == "unknown" then
		          my_directory_name = identity.phone
