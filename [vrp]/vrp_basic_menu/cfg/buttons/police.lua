			  ...
			  if target ~= nil then
                ...
                local handcuffed = vRPclient.isHandcuffed(target)  
                  if handcuffed then 
					...
					TriggerEvent("logs:serverLog", "ID "..user_id.." enviou o ID "..target_id.." para a cadeia por "..jail_time.." anos.", "policia", false)
			      else
				    vRPclient.notify(player,lang.police.not_handcuffed())
			      end
			  else
			...
			if custom ~= nil then
			  ...
			  if tonumber(custom) > 0 or vRP.hasPermission(user_id,lang.unjail.admin()) then
				...
				if target ~= nil then
				  ...
				  TriggerEvent("logs:serverLog", "ID "..user_id.." removeu o ID "..target_id.." da cadeia.", "policia", false)
				else
				  vRPclient.notify(player,lang.common.invalid_value())
				end
			  else
			    ...
			    if reason ~= nil and reason ~= "" then 
				  ...
				  if target ~= nil then
		            ...
		            if vRP.tryFullPayment(tonumber(target_id), tonumber(fine)) then
					  ...
					  TriggerEvent("logs:serverLog", "ID "..user_id.." multou o ID "..target_id.." por "..fine.." pelo motivo "..reason, "policia", false)
...
ch_handcuff = {function(player,choice)
    ...
    if nuser_id ~= nil then
	  ...
	  TriggerEvent("logs:serverLog", "ID "..user_id.." algemou "..nuser_id..".", "policia", false)
