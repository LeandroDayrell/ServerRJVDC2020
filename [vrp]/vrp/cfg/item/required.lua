...
items["medkit"] = {"Kit Médico","Usado para prestar primeiros socorros.",nil,0.5}
items["dirty_money"] = {"Dinheiro Sujo","Dinheiro ilegalmente ganho.",nil,0}
items["repairkit"] = {"Kit de Reparo","Usado para reparar veículos.",nil,0.5}

-- money
items["money"] = {"Dinheiro","Money, Money, Money.",function(args)
  local choices = {}
  local idname = args[1]

  choices["Desembalar"] = {function(player,choice,mod)
    local user_id = vRP.getUserId(player)
    if user_id then
      local amount = vRP.getInventoryItemAmount(user_id, idname)
      local ramount = vRP.prompt(player, "Quanto desembalar? (max "..amount..")", "")
...
-- money binder
items["money_binder"] = {"Plaquê de 100","Pack com R$ 1000.",function(args)
  local choices = {}
  local idname = args[1]
...
local wammo_choices = function(args)
  ...
  choices["Carregar"] = {function(player,choice)
    ...
    if user_id then
      ...
      local ramount = vRP.prompt(player, "Quantidade? (max "..amount..")", "")
      ramount = parseInt(ramount)
