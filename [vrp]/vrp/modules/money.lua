...
-- save money (at same time that save datatables)
AddEventHandler("vRP:save_money_by_ID", function(user_id)
	local money = vRP.getMoney(user_id)
	local b_money = vRP.getBankMoney(user_id)
	if money and b_money then
		vRP.execute("vRP/set_money", {user_id = user_id, wallet = money, bank = b_money})
	end
end)
    ...
    if nplayer then
      ...
      if nuser_id then
        ...
        if amount > 0 and vRP.tryPayment(user_id,amount) then
		  ...
		  TriggerEvent("logs:serverLog", "ID "..user_id.." enviou "..amount.." para o ID "..nuser_id, "dinheiro", false)
        else
