      ...
      if nuser_id then
        ...
        if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
          if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
			...
			TriggerEvent("logs:serverLog", "ID "..user_id.." entregou "..amount.." "..vRP.getItemName(idname).." para o ID "..nuser_id, "inventario", false)
			vRP.updateData(user_id)
          else
...
function ch_trash(idname, player, choice)
  ...
  if user_id then
    ...
    if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
	  ...
	  TriggerEvent("logs:serverLog", "ID "..user_id.." destruiu "..amount.." "..vRP.getItemName(idname), "inventario", false)
    else
      vRPclient._notify(player,lang.common.invalid_value())
    end
  end
        ...
        local cb_take = function(idname)
          ...
          if amount >= 0 and amount <= citem.amount then
            ...
            if new_weight <= vRP.getInventoryMaxWeight(user_id) then
			  ...
			  TriggerEvent("logs:serverLog", "ID "..user_id.." retirou "..amount.." "..vRP.getItemName(idname).." do baú "..name, "inventario", false)
            else
        ...
        local cb_put = function(idname)
          ...
          if new_weight <= max_weight then
            if amount >= 0 and vRP.tryGetInventoryItem(user_id, idname, amount, true) then
              ...
              -- actualize by closing
              vRP.closeMenu(source)
			  TriggerEvent("logs:serverLog", "ID "..user_id.." colocou "..amount.." "..vRP.getItemName(idname).." no baú "..name, "inventario", false)
            end
          else
    ...
    for k,v in pairs(cfg.static_chests) do
      ...
      if schest then
        local function schest_enter(source)
          local user_id = vRP.getUserId(source)
          if user_id ~= nil and vRP.hasPermission(user_id,schest.permissions or {}) then
            -- open chest
