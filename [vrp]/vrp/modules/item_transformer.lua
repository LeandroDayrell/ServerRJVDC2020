    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        if new_weight > vRP.getInventoryMaxWeight(user_id) then
          inventory_ok = false
          vRPclient.notify(tonumber(k), {"Inventario Full"})
        end

		if not money_ok then
          vRPclient.notify(tonumber(k), {"Dinheiro Insuficiente"})
		end

		if not reagents_ok then
          vRPclient.notify(tonumber(k), {"Falta alguma coisa durro."})
		end
...
function vRP.setItemTransformer(name,itemtr)
  ...
  for action,recipe in pairs(tr.itemtr.recipes) do
    ...
    info = info.."<br /><span style=\"color: rgb(255,255,255)\">=></span>"
	...
	if player ~= nil then
		local player = vRP.getUserSource(user_id)
			  if vRP.inArea(user_id, "vRP:tr:"..tr.name) then
...
local function informer_placement_tick()
  ...
  for k,v in pairs(vRP.rusers) do
    ...
    vRPclient.setNamedMarker(player,{"vRP:informer",x,y,z,-0.87,0.7,0.7,0.5,255,140,0,125,150})
