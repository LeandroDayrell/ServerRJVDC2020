...
local function tr_add_player(tr,player,action,recipe) -- add player to transforming
  tr.players[player] = action -- reference player as using transformer
  vRP.closeMenu(player)
    ...
    if v and user_id then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        if money_ok and reagents_ok and inventory_ok then -- do transformation
		  ...
		  -- play anim
		  if not playingAnim[tonumber(k)] then
			if tr.itemtr.anim then
				vRPclient._playAnim(tonumber(k),true,tr.itemtr.anim,true)
				playingAnim[tonumber(k)] = true
			end
		  end
        else
		  vRPclient.stopAnim(tonumber(k),false)
		  playingAnim[tonumber(k)] = false
		end
      end
    end
  end
  ...
  for action,recipe in pairs(tr.itemtr.recipes) do
    ...
    tr.menu[action] = {function(player,choice)
      if vRP.inArea(player, "vRP:tr:"..tr.name) then
        tr_add_player(tr,player,action,recipe.description)
      end
