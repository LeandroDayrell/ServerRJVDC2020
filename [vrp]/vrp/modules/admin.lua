...
local function ch_whitelist(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.whitelist") then
    local id = vRP.prompt(player,"ID para adicionar na Whitelist: ","")
    id = parseInt(id)
    vRP.setWhitelisted(id,true)
    vRPclient._notify(player, id.. " setado na Whitelist")
	TriggerEvent("logs:serverLog", "ID "..user_id.." adicionou o ID "..id.." na whitelist", "admin_whitelist", false)
  end
end
...
local function ch_unwhitelist(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.unwhitelist") then
    local id = vRP.prompt(player,"ID para remover na Whitelist: ","")
    id = parseInt(id)
    vRP.setWhitelisted(id,false)
    vRPclient._notify(player, id.. " removido da Whitelist")
	TriggerEvent("logs:serverLog", "ID "..user_id.." removeu o ID "..id.." na whitelist", "admin_whitelist", false)
  end
end
...
local function ch_addgroup(player,choice)
  ...
  if user_id ~= nil and vRP.hasPermission(user_id,"player.group.add") then
    local id = vRP.prompt(player,"ID: ","") 
    id = parseInt(id)
    local group = vRP.prompt(player,"Grupo a adicionar: ","")
    if group then
      vRP.addUserGroup(id,group)
      vRPclient._notify(player, group.." adicionado ao ID "..id)
	  TriggerEvent("logs:serverLog", "ID "..user_id.." setou "..id.." no grupo "..group, "admin_grupos", false)
    end
  end
end
...
local function ch_removegroup(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.group.remove") then
    local id = vRP.prompt(player,"ID: ","")
    id = parseInt(id)
    local group = vRP.prompt(player,"Grupo a remover: ","")
    if group then
      vRP.removeUserGroup(id,group)
      vRPclient._notify(player, group.." removido do ID "..id)
	  TriggerEvent("logs:serverLog", "ID "..user_id.." removeu o "..id.." no grupo "..group, "admin_grupos", false)
    end
  end
end
...
local function ch_kick(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.kick") then
    local id = vRP.prompt(player,"ID para kickar: ","")
    id = parseInt(id)
    local reason = vRP.prompt(player,"Motivo: ","")
    local source = vRP.getUserSource(id)
    if source then
      vRP.kick(source,reason)
      vRPclient._notify(player, "Kickou o ID "..id)
	  TriggerEvent("logs:serverLog", "ID "..user_id.." kickou o "..id.." pelo motivo: "..reason, "admin", false)
    end
  end
end
...
local function ch_ban(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.ban") then
    local id = vRP.prompt(player,"ID para banir: ","")
    id = parseInt(id)
    local reason = vRP.prompt(player,"Motivo: ","")
    local source = vRP.getUserSource(id)
    if source then
      vRP.ban(source,reason)
      vRPclient._notify(player, "Baniu o ID "..id)
	  TriggerEvent("logs:serverLog", "ID "..user_id.." baniu o "..id.." pelo motivo: "..reason, "admin", false)
    end
  end
end
...
local function ch_unban(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.unban") then
    local id = vRP.prompt(player,"ID para desbanir: ","")
    id = parseInt(id)
    vRP.setBanned(id,false)
    vRPclient._notify(player, "Desbanido o ID "..id)
	TriggerEvent("logs:serverLog", "ID "..user_id.." desbaniu o "..id, "admin", false)
  end
end
...
local function ch_coords(player,choice)
  ...
  vRP.prompt(player,"Copie as coordenadas usando Ctrl-A Ctrl-C",x..","..y..","..z)
end
...
local function ch_givemoney(player,choice)
  ...
  if user_id then
	...
	TriggerEvent("logs:serverLog", "ID "..user_id.." spawnou R$"..amount, "admin", false)
  end
end
...
local function ch_giveitem(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id then
    local idname = vRP.prompt(player,"Nome do item:","")
    idname = idname or ""
    local amount = vRP.prompt(player,"Quantidade:","")
    amount = parseInt(amount)
    vRP.giveInventoryItem(user_id, idname, amount,true)
	TriggerEvent("logs:serverLog", "ID "..user_id.." spawnou "..amount.." "..idname, "admin", false)
  end
end
...
local function ch_calladmin(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id then
    local desc = vRP.prompt(player,"Diga qual o problema:","") or ""
	if desc ~= "" then
		...
		for k,v in pairs(players) do
		  async(function()
			local ok = vRP.request(v,"Ticket ADM (ID = "..user_id..") está solicitando ajuda: "..htmlEntities.encode(desc), 60)
			if ok then -- take the call
			  if not answered then
				-- answer the call
				vRPclient._notify(player,"Um ADM atendeu seu ticket.")
				vRPclient._teleport(v, vRPclient.getPosition(player))
				answered = true
			  else
				vRPclient._notify(v,"Ticket já atendido.")
			  end
			end
		  end)
		end
	else
		vRPclient._notify(player,"~r~Coloque uma descrição para o problema")
	end
  end
end
