...
function vRP.registerDBDriver(name, on_init, on_prepare, on_query)
  if not db_drivers[name] then
    ...
    if name == config.db.driver then -- use/init driver
      ...
      if ok then
        print("[vRP] Conectado a DataBase pelo \""..name.."\".")
        db_initialized = true
...
function vRP.dropPlayer(source)
  ...
  if user_id then
    ...
    -- print("[vRP] "..endpoint.." desconectou (user_id = "..user_id..")")
	TriggerEvent("logs:serverLog", "[vRP] "..endpoint.." saiu (user_id = "..user_id..")", "sistema", true)
...
function vRP.updateData(user_id)
  TriggerEvent("vRP:save_money_by_ID", user_id)

  Debug.log("save "..user_id)
  for k,v in pairs(vRP.user_tables) do
	if k == user_id then
	  vRP.setUData(k,"vRP:datatable",json.encode(v))
	end
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
  ...
  if ids ~= nil and #ids > 0 then
    deferrals.update("[vRP] Checando identificadores...")
    ...
    if user_id then -- check user validity 
      deferrals.update("[vRP] Checando banimento...")
      if not vRP.isBanned(user_id) then
        deferrals.update("[vRP] Checando whitelist...")
        if not config.whitelist or vRP.isWhitelisted(user_id) then
          if vRP.rusers[user_id] == nil then -- not present on the server, init
            ...
            deferrals.update("[vRP] Carregando base de dados...")
            ...
            deferrals.update("[vRP] Buscando ultimo login...")
            ...
            -- trigger join
			TriggerEvent("logs:serverLog", "[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") entrou (user_id = "..user_id..")", "sistema", true)
          ...
          print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejeitado: sem whitelist. (user_id = "..user_id.."")
          Citizen.Wait(1000)
          deferrals.done("[vRP] Sem whitelist. Faça sua whitelist no discord. Seu ID: "..user_id..".")
        end
      else
        print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejeitado: banido (user_id = "..user_id..")")
        Citizen.Wait(1000)
        deferrals.done("[vRP] Banido. Seu ID: "..user_id..".")
      end
    else
      print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejeitado: erro de identificação")
      Citizen.Wait(1000)
      deferrals.done("[vRP] Erro de identificação.")
    end
  else
    print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejeitado: faltando identificadores")
    Citizen.Wait(1000)
    deferrals.done("[vRP] Faltando identificadores.")
  end
end)
...
-- TriggerEvent("logs:serverLog", "ID "..user_id.." recebeu ", "inventory", false)
AddEventHandler('logs:serverLog', function(text,file_name,console)
	local currentDatetime = os.date("%d/%m/%Y %H:%M:%S")
	local formattedLog = string.format("[%s] %s",currentDatetime,text)
	if config.logs_to_discord == false then
		local logFile,errorReason = io.open(file_name..".log","a")
		if not logFile then return print(errorReason) end
		logFile:write(formattedLog.."\n")
		logFile:close()
	elseif config.logs_to_discord == true then
		TriggerEvent('logs:ToDiscord', config.discord_webhook , "Logs", text, "https://www.tumarcafacil.com/wp-content/uploads/2017/06/RegistroDeMarca-01-1.png", false, false)
	end
	-- Print to console
	if console then
		print(formattedLog)
...
-- TriggerEvent('logs:ToDiscord', config.discord_webhook , "Logs", text, https://www.tumarcafacil.com/wp-content/uploads/2017/06/RegistroDeMarca-01-1.png, false, false)
RegisterServerEvent('logs:ToDiscord')
AddEventHandler('logs:ToDiscord', function(WebHook, Name, Message, Image, External, TTS)
