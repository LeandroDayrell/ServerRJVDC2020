...
emp = {}
Tunnel.bindInterface("entrega_marfim",emp) 
vRPclient = Tunnel.getInterface("vRP")
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNÇÕES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent('entrega_marfim:permissao')
AddEventHandler('entrega_marfim:permissao',function()
	local source = source
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,"Amarelos") then
	    TriggerClientEvent('entrega_marfim:permissao', player)
	end
end)

RegisterServerEvent('entrega_marfim:itensReceber')
AddEventHandler('entrega_marfim:itensReceber', function(quantidade)
	local src = source
    ...
    if user_id then
        ...
        if new_weight <= vRP.getInventoryMaxWeight(user_id) then
            if vRP.tryGetInventoryItem(user_id,"marfim",quantidade,true) then
