...
emp = Tunnel.getInterface("entrega_marfim")
...
-- INICIANDO TRABALHO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('entrega_marfim:permissao')
	...
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 696.38690185547,-305.33325195313,59.247798919678,true) <= 1 then
            ...
            if IsControlJustPressed(0,38) then	
				TriggerEvent('entrega_marfim:permissao') 
			end
		end
		if emservico then
			...
			if statuses then
				...
				drawTxt(ui.right_x+0.050,ui.bottom_y-0.058,1.0,1.0,0.45,"ENTREGUE ~g~"..quantidade.."~w~ MARFIM",255,255,255,255)
			else
			...
			if distance <= 50 then
				...
				if distance < 3 then
                    ...
                    if IsControlJustPressed(0,101) then
                        ...
                        RemoveBlip(blip)
                        TriggerServerEvent('entrega_marfim:itensReceber', quantidade)
                        while true do
