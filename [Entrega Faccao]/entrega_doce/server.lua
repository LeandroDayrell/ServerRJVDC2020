...
emp = {}
Tunnel.bindInterface("entrega_doce",emp) 
vRPclient = Tunnel.getInterface("vRP")
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNÇÕES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent('entrega_doce:permissao')
AddEventHandler('entrega_doce:permissao',function()
	local source = source
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,"Amarelos") then
	    TriggerClientEvent('entrega_doce:permissao', player)
	end
end)

RegisterServerEvent('entrega_doce:itensReceber')
AddEventHandler('entrega_doce:itensReceber', function(quantidade)
	local src = source
    ...
    if user_id then
        ...
        if new_weight <= vRP.getInventoryMaxWeight(user_id) then
            if vRP.tryGetInventoryItem(user_id,"doce",quantidade,true) then
