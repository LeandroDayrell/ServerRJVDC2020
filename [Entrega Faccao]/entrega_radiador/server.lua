...
emp = {}
Tunnel.bindInterface("entrega_radiador",emp) 
vRPclient = Tunnel.getInterface("vRP")
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNÇÕES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent('entrega_radiador:permissao')
AddEventHandler('entrega_radiador:permissao',function()
	local source = source
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,"Amarelos") then
	    TriggerClientEvent('entrega_radiador:permissao', player)
	end
end)

RegisterServerEvent('entrega_radiador:itensReceber')
AddEventHandler('entrega_radiador:itensReceber', function(quantidade)
	local src = source
    ...
    if user_id then
        ...
        if new_weight <= vRP.getInventoryMaxWeight(user_id) then
            if vRP.tryGetInventoryItem(user_id,"radiador",quantidade,true) then
