function CloseShop() {
  $(".container-item").html('');
	$("#carshop").hide();
    $.post('http://vrp_caminhao/fechar', JSON.stringify({}));
}

$(document).ready(function(){

    $("#fechar").click(function(){
      $.post('http://vrp_caminhao/fechar', JSON.stringify({}));2
      $(".container-item").html('');
      $("#carshop").hide();
    });

    $(".container-item").on('click', '#comprar', function () {
        $.post('http://vrp_caminhao/comprar', JSON.stringify({id: $(this).data('id')}));
        $(".container-item").html('');
        $("#carshop").hide();
    });
