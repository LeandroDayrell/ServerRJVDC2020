...
local selecionado = 0
local parte = 0
local CoordenadaX = -589.71
local CoordenadaY = -936.66
...
Citizen.CreateThread(function()
	while true do
		...
		if distance <= 15 then
		    ...
		    DrawText3D(CoordenadaX,CoordenadaY,CoordenadaZ+0.33, "~b~Globo News", 0.9, 1)
		...
		if servico and parte == 0 and not holdingCam then
			local ped = PlayerPedId()
			local x,y,z = table.unpack(GetEntityCoords(ped))
