...
SVjob = Tunnel.getInterface("garagem_bope")
...
RegisterNetEvent('Policia:Receberbope')
AddEventHandler('Policia:Receberbope', function(veiculos)
...
function SpawnGaragem(vtype)
	  ...
	  if HasModelLoaded(mhash) then
        for k,v in pairs(dp) do
			...
			SetVehicleNumberPlateText(nveh, "R-bope")
        ...
        for k,v in pairs(dp) do     
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garagem.x, garagem.y, garagem.z, true ) <= 1 and display then
                ...
                if IsControlJustReleased(1, 51) then
                    TriggerServerEvent('Policia:Enviarbope')
                end
            end
        end
        Wait(1)
