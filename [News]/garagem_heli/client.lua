...
SVjob = Tunnel.getInterface("garagem_heli")
...
RegisterNetEvent('Policia:ReceberHeli')
AddEventHandler('Policia:ReceberHeli', function(veiculos)
...
function SpawnGaragem(vtype)
	  ...
	  if HasModelLoaded(mhash) then
        for k,v in pairs(dp) do
			...
			SetVehicleNumberPlateText(nveh, "R-HELI")
    ...
    while true do
        for k,v in pairs(dp) do     
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garagem.x, garagem.y, garagem.z, true ) <= 1 and display then
                DrawText3D(garagem.x, garagem.y, garagem.z, "Aperte [~g~E~w~] para acessar a garagem de helicopteros")
                if IsControlJustReleased(1, 51) then
                    TriggerServerEvent('Policia:EnviarHeli')
                end
            end
        end
        Wait(1)
